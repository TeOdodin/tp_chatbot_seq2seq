...
class Seq2Seq(object):
    ...
    def __init__(self, xseq_len, yseq_len, 
        ...
        self.vocab_size = emb_dim * xseq_len
        ...
        def __graph__():
            ...
            self.enc_ip = [ tf.keras.Input(name='ei_{}'.format(t), shape=(None,), dtype=tf.dtypes.int64) for t in range(xseq_len) ]

            #  labels that represent the real outputs
            self.labels = [ tf.keras.Input(name='ei_{}'.format(t), shape=(None,), dtype=tf.dtypes.int64) for t in range(yseq_len) ]
            ...
            self.keep_prob = tf.keras.layers.LSTM(64)
            # define the basic cell
            basic_cell = tf.keras.layers.LSTMCell(emb_dim, recurrent_dropout = 64/emb_dim)
            # stack cells together : n layered model
            stacked_lstm = tf.keras.layers.StackedRNNCells([basic_cell]*num_layers)
            ...
            with tf.compat.v1.variable_scope('decoder') as scope:
                ...
                embedding_layer = tf.keras.layers.Embedding(self.vocab_size, emb_dim)
                sampler = tfa.seq2seq.TrainingSampler()
                output_layer = tf.keras.layers.Dense(self.vocab_size)

                decoder = tfa.seq2seq.BasicDecoder(stacked_lstm, sampler, output_layer)

                input_ids = tf.random.uniform(
                    [xseq_len, epochs], maxval=self.vocab_size, dtype=tf.dtypes.int64)
                input_lengths = tf.fill([xseq_len], epochs)
                input_tensors = embedding_layer(input_ids)
                initial_state = decoder_cell.get_initial_state(input_tensors)

                self.decode_outputs, self.decode_states, lengths = decoder(
                    input_tensors, sequence_length=input_lengths, initial_state=initial_state)
                # self.decode_outputs, self.decode_states = tfa.seq2seq.BasicDecoder(self.enc_ip,self.dec_ip, stacked_lstm,
                ...
                decoder = tfa.seq2seq.BasicDecoder(stacked_lstm, sampler, output_layer)
                self.decode_outputs_test, self.decode_states_test, lengths = decoder(
                    input_tensors, sequence_length=input_lengths, initial_state=initial_state)
                # self.decode_outputs_test, self.decode_states_test = tfa.seq2seq.BasicDecoder(
            ...
            self.loss = tfa.legacy_seq2seq.sequence_loss(self.decode_outputs, self.labels, loss_weights, yvocab_size)
